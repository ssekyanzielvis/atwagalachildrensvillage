...
export default function BankDetailsPage() {
  ...
  const [editingDetail, setEditingDetail] = useState<BankDetails | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingDetail) {
        ...
        const { error } = await (supabase.from('bank_details') as any).insert(formData);

        if (error) throw error;
        showNotification('Bank details created successfully', 'success');
      }
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const { error } = await (supabase.from('bank_details') as any).delete().eq('id', id);
  ...
  const handleToggleActive = async (id: string, currentStatus: boolean) => {
    try {
      ...
      if (error) throw error;
      showNotification(`Bank details ${!currentStatus ? 'activated' : 'deactivated'}`, 'success');
      fetchBankDetails();
    } catch (error: any) {
      showNotification(error.message, 'error');
    }
  ...
  const handleEdit = (detail: BankDetails) => {
    ...
    setFormData({
      ...
      intermediate_bank: detail.intermediate_bank || '',
  ...
  if (loading && bankDetails.length === 0) {
    return (
    ...
    <div className="p-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-600 mt-1">Manage direct bank transfer information</p>
        </div>
        <button
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {bankDetails.map((detail) => (
              <tr key={detail.id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="text-gray-900">{detail.account_number}</div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="text-gray-600">{detail.display_order}</div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <button
          ...
          <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center p-6 border-b">
              <h2 className="text-2xl font-bold">
                {editingDetail ? 'Edit Bank Details' : 'Add Bank Details'}
              </h2>
